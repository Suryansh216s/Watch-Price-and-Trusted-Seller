...
# Custom CSS to fix input selection issue and improve responsiveness
st.markdown("""
<style>
...
def main():
    ...
    if submitted:
        try:
            ...
            df_transformed = scaler.transform(pt.transform(df_input))
            
            price_prediction = model_xgb.predict(df_transformed)[0]
            trusted_prediction = model_lr_t.predict(df_transformed)[0]

            st.subheader("Predicted Price 💰")
            st.success(f"${price_prediction:,.2f}")

            st.subheader("Seller Trustworthiness 🤝")
            if trusted_prediction >= 0.5:
                st.success("Trusted Seller ✅")
            else:
                st.warning("Not a Trusted Seller ❌")
