...
def main():
    ...
    if submitted:
        try:
            ...
            # Predict price
            price_pred_scaled = model_xgb.predict(input_df)  # This is a 1D array
            price_pred_scaled_2d = price_pred_scaled.reshape(-1, 1)  # Convert to 2D array
            price_pred = pt.inverse_transform(price_pred_scaled_2d)[0][0]  # Apply inverse transform
            
            # Predict seller trustworthiness
            ...
            # st.write("Scaled Features:", input_df[numerical_features])
            # st.write("Predicted Price Shape:", model_xgb.predict(input_df).shape)
